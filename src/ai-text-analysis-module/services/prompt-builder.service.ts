export class PromptBuilderService {
    build(params: {
        ...
        return [
            ...
            "- text_units should prefer single lexical units. Use short lexical phrases only when the whole phrase should be translated as one unit.",
            "- do not combine an article and a noun into one text_unit unless the pair.source_text will use the same full phrase.",
            ...
            "",
            "Rules for pairs:",
            "- CRITICAL: every pair.source_text must exactly equal each text_units item referenced by occurrence_indexes.",
            "- Never create a pair for only part of a text_units item.",
            '- Invalid example: text_units[0] = "The morning" and pair.source_text = "The" with occurrence_indexes [0].',
            '- Valid option 1: text_units[0] = "The morning" and pair.source_text = "The morning" with occurrence_indexes [0].',
            '- Valid option 2: text_units = ["The", "morning"] and pair.source_text = "The" with occurrence_indexes [0].',
            "- If a word should be translated separately, it must be a separate item in text_units.",
            "- If text_units contains a phrase, the pair must use the same full phrase as source_text.",
    ...
    buildRetryPrompt(params: {
        ...
        return [
            ...
            "Do not keep the same invalid segmentation.",
            "If text_units at the reported index is a phrase, either change pair.source_text to the full phrase or split text_units into separate lexical units.",
            "Never point occurrence_indexes to a text_units item that only contains pair.source_text as a substring.",
            'For example, if text_units[0] is "The morning", pair.source_text must not be "The" with occurrence_indexes [0].',
